...
const Settings = () => {
  ...
  const fetchMerchantData = async () => {
    ...
    try {
      ...
      if (response.data && response.data.merchant) {
        ...
        // Initialize fee form
        setFeeForm({
          feeChargedTo: response.data.merchant.feeChargedTo || 'customer'
        });
      }
    } catch (error) {
      console.error('Error fetching merchant data:', error);
  ...
  const handleFeeFormSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${URL}/api/merchants/profile`, feeForm, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('access_token')}`,
          'x-api-key': user?.apiKey || '',
          'x-api-secret': user?.apiSecret || ''
      ...
      if (response.data && response.data.success) {
        setMerchantData(prev => ({
          ...prev,
          feeChargedTo: feeForm.feeChargedTo
        }));
        
        setSuccessMessage('Fee settings updated successfully!');
      ...
      setErrorMessage('Failed to update fee settings. Please try again.');
    } finally {
      setUpdating(false);
    }
  };
...
## Fee Configuration
Your account is currently configured to charge fees to: ${merchantData?.feeChargedTo === 'customer' ? 'customers' : 'merchants (you)'}

${merchantData?.feeChargedTo === 'customer' ? 
  '- Customer pays the listed amount + 1.75% processing fee\n- You receive the full listed amount' :
  '- Customer pays exactly the listed amount\n- You receive the listed amount minus 1.75% processing fee'
}
...
\`\`\`json
{
  ...
  "payment": {
    "id": "payment_id",
    "amount": 100.00,
    "totalAmount": ${merchantData?.feeChargedTo === 'customer' ? '101.75' : '100.00'},
    "feeAmount": 1.75,
    "currency": "USDT",
    "status": "pending",
    "feeChargedTo": "${merchantData?.feeChargedTo || 'customer'}"
  },
  "paymentUrl": "https://coinley.app/pay/payment_id"
}
\`\`\`
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-md p-4">
              <div className="space-y-1">
                ...
                >
                  <RiPercentLine className="mr-3 text-lg" />
          ...
          <div className="lg:col-span-3">
            ...
            {activeTab === 'fees' && (
              <div className="bg-white rounded-lg shadow-md p-6">
                <h2 className="text-xl font-semibold mb-6 flex items-center">
                  <RiPercentLine className="mr-2 text-[#7042D2]" />
                  Fee Settings
                </h2>
                
                <div className="bg-blue-50 p-4 mb-6 rounded-lg text-sm text-blue-800 flex items-start">
                  <RiInformationLine className="text-blue-600 mt-1 mr-2 flex-shrink-0" />
                  <div>
                    <p className="font-medium mb-1">Transaction Fee: 1.75%</p>
                    <p>
                      Choose whether transaction fees are charged to your customers or deducted from your revenue.
                      This setting affects all new payments created after the change.
                    </p>
                  </div>
                </div>
                ...
                <form onSubmit={handleFeeFormSubmit}>
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {/* Customer Pays Fee Option */}
                      <div 
                        className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                          feeForm.feeChargedTo === 'customer' 
                            ? 'border-[#7042D2] bg-purple-50' 
                            : 'border-gray-200 hover:border-gray-300'
                        }`}
                        onClick={() => handleFeeFormChange({ target: { name: 'feeChargedTo', value: 'customer' } })}
                      >
                        <div className="flex items-start">
                          <input
                            ...
                            name="feeChargedTo"
                            value="customer"
                            checked={feeForm.feeChargedTo === 'customer'}
                            onChange={handleFeeFormChange}
                            className="mt-1 mr-3 text-[#7042D2] focus:ring-[#7042D2]"
                          />
                          <div className="flex-1">
                            <div className="flex items-center mb-2">
                              <RiMoneyDollarCircleLine className="text-[#7042D2] mr-2" />
                              <h3 className="font-medium text-gray-800">Customer Pays Fee</h3>
                            </div>
                            <p className="text-sm text-gray-600 mb-3">
                              Transaction fees are added to the payment amount and charged to your customers.
                            </p>
                            <div className="bg-gray-50 p-3 rounded text-xs">
                              <strong>Example:</strong><br />
                              Product: $100.00<br />
                              Fee (1.75%): +$1.75<br />
                              <strong>Customer pays: $101.75</strong><br />
                              <strong>You receive: $100.00</strong>
                            </div>
                          </div>
                        </div>
                      </div>

                      {/* Merchant Pays Fee Option */}
                      <div 
                        className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                          feeForm.feeChargedTo === 'merchant' 
                            ? 'border-[#7042D2] bg-purple-50' 
                            : 'border-gray-200 hover:border-gray-300'
                        }`}
                        onClick={() => handleFeeFormChange({ target: { name: 'feeChargedTo', value: 'merchant' } })}
                      >
                        <div className="flex items-start">
                          <input
                            type="radio"
                            name="feeChargedTo"
                            value="merchant"
                            checked={feeForm.feeChargedTo === 'merchant'}
                            onChange={handleFeeFormChange}
                            className="mt-1 mr-3 text-[#7042D2] focus:ring-[#7042D2]"
                          />
                          <div className="flex-1">
                            <div className="flex items-center mb-2">
                              <RiStoreLine className="text-[#7042D2] mr-2" />
                              <h3 className="font-medium text-gray-800">Merchant Pays Fee</h3>
                            </div>
                            <p className="text-sm text-gray-600 mb-3">
                              Transaction fees are deducted from your revenue. Customers pay exactly the listed price.
                            </p>
                            <div className="bg-gray-50 p-3 rounded text-xs">
                              <strong>Example:</strong><br />
                              Product: $100.00<br />
                              ...
                              <strong>You receive: $98.25</strong>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center justify-between">
                        <div>
                          <h4 className="font-medium text-gray-800">Current Setting</h4>
                          <p className="text-sm text-gray-600">
                            {merchantData?.feeChargedTo === 'customer' 
                              ? 'Transaction fees are currently charged to customers' 
                              : 'Transaction fees are currently paid by you (merchant)'
                            }
                          </p>
                        </div>
                        <div className={`px-3 py-1 rounded-full text-xs font-medium ${
                          ...
                          {merchantData?.feeChargedTo === 'customer' ? 'Customer Pays' : 'Merchant Pays'}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-6 flex justify-end">
                      <button
                        type="submit"
                        disabled={updating || feeForm.feeChargedTo === merchantData?.feeChargedTo}
                        className="px-6 py-2 bg-[#7042D2] text-white rounded-md hover:bg-opacity-90 transition-all disabled:opacity-50 flex items-center gap-2"
                      >
                        {updating ? (
                          <>
